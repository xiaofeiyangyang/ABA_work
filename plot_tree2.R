...
# cluster genes and species
 ...
 cairo_pdf('figure4A_test.pdf', width = 20)
...
UGTEcm <- read.table("112UGT_0.txt", sep = "\t", header = TRUE, stringsAsFactor = FALSE)
...
speccol <- c("#F8776D", "#00C181", "#00B9E3", "#DB72FB", "#93AA00")

#names(speccol) <- c("Animals", "Fungi", "Plants", "Protists","Bacteria")
#names(speccol) <- NULL

kingdom <- read.table("271euk_taxonomy6.txt", sep = "\t", header = FALSE, stringsAsFactor = FALSE)
specName <- kingdom$V1
phy <- kingdom$V2
for (i in 1: length(phy)){
	...
	if ( phy[i] == "Animals"){
		...
		phy[i] <- speccol[2]
	} else if( phy[i] == "Plants"){
		phy[i] <- speccol[3]
	} else if( phy[i] == "Protists"){
		phy[i] <- speccol[4]
	} else if(phy[i] == "Bacteria"){
    phy[i] <- speccol[5]
  }else{}
}
domain <- speccol
names(domain) <- c("Animals", "Fungi", "Plants", "Protists","Bacteria")
names(phy) <- specName
...
#plot 4A
cairo_pdf('figure4A_update.pdf', width = 20)
ATPphyloPlot2 <- PlotPhyloProfile2(phyloData, speCol = tip_name, geneCol = colGene,
classCol = domain, tipLabel = tip_label, legend.position = 'left',  
                           widthsShinkage = c(0.3, 0.1, 9, 2.2),
                             heightsShinkage = c(0.3, 0.03, 0.05, 0.3),)
...
#plot 3A
cairo_pdf('figure3A_update.pdf', width = 20)
ATPphyloPlot2 <- PlotPhyloProfile2(phyloData, speCol = tip_name, geneCol = colGene,
classCol = domain, tipLabel = tip_label, legend.position = 'left',  
                           widthsShinkage = c(0.3, 0.1, 9, 2.2),
                             heightsShinkage = c(0.1, 0.015, 0.01, 0.3),)
dev.off()

#plot 3A
cairo_pdf('figure2A_update.pdf', width = 20)
ATPphyloPlot2 <- PlotPhyloProfile2(phyloData, speCol = tip_name, geneCol = colGene,
classCol = domain, tipLabel = tip_label, legend.position = 'left',  
                           widthsShinkage = c(0.3, 0.1, 9, 2.2),
                             heightsShinkage = c(0.1, 0.015, 0.01, 0.3),)
...
#~~~~~~~~~~~~~~~~~~~deal with new Taxa~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
tax <- read.table("271euk_taxonomy4.txt", sep = "\t", header = FALSE, stringsAsFactor = FALSE)
newTable <- read.csv("wKEGGSpe.csv", header = TRUE, stringsAsFactor = FALSE)

newTax <- newTable[newTable$abbName %in% tax$V1,]
phy <- strsplit(newTax$Phylo,";")
phy <- sapply(phy, function(x) x[2])

newTax$phy <- phy
newTax <- newTax[-2:-6]


write.table(newTax,"271euk_taxonomy6.txt", sep = "\t", row.names=FALSE, col.names  = FALSE,quote = FALSE)